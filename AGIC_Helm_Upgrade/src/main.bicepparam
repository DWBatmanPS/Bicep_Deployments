using './main.bicep' /*Provide a path to a bicep template*/

param aksClusterName = 'danwagicaks'
param aksClusterNodeCount = 2
param aksClusterNodeSize = 'Standard_D2s_v3'
param aksClusterKubernetesVersion = '1.29.4'
param aksdnsPrefix = 'danwagicaksdns'
param linuxadmin = 'danadmin'
param sshkey = 'ssh-rsa  danwheeler@LAPTOP-3DALUHP4'
param VnetName = 'danvnet'
param virtualNetwork_AddressPrefix = '10.0.0.0/16'
param subnet_Names = [
  'AKSSubnet'
  'AppGatewaySubnet'
]
param akspodcidr = '10.244.0.0/16'
param aksserviceCidr = '192.168.16.0/20'
param aksinternalDNSIP = '192.168.16.10'
param appgwname = 'agic-appgw'
param publicIP_ApplicationGateway_Name = 'agic-appgwip'
param serviceprincipal_client_Id = '88d8a2d9-e8fa-43e4-933d-1bd0f8ab8566'
