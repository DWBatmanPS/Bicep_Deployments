...
param vanityDomainName string = 'contoso.com'

@description('Rewrite rule sets for the Application Gateway, passed in as input so environment-specific rules stay out of source control. Use the {backendURL} and {region} tokens in patterns/values; they are substituted at deploy time.')
...
var httpListeners = (usePrivateFrontend) ? [
      {
        ...
        properties: {
          ...
          hostNames: []
          requireServerNameIndication: (nossl) ? false : true
          ...
          hostName: (nossl) ? null : vanityDomainName
        }
      }
    ] : (useBothPrivateAndPublicFrontend) ? [
      {
  ...
  var hasRewriteRules = !empty(rewriteRuleSets)
  var rewriteRuleSetName = hasRewriteRules ? rewriteRuleSets[0].name : ''
  var resolvedRewriteRuleSets = hasRewriteRules ? json(replace(replace(string(rewriteRuleSets), '{backendURL}', backendURL), '{region}', region)) : []
  var rewriteRuleSetRef = hasRewriteRules ? {
    id: resourceId('Microsoft.Network/applicationGateways/rewriteRuleSets', applicationGateway_Name, rewriteRuleSetName)
  } : null
  ...
  var routingrules = (pathmap) ? [
    ...
    ] : (useBothPrivateAndPublicFrontend) ? [
      {
        ...
        properties: {
          ...
          rewriteRuleSet: rewriteRuleSetRef
        }
      }
      {
...
resource applicationGateway 'Microsoft.Network/applicationGateways@2022-11-01' = {
  ...
  properties: {
    ...
    urlPathMaps: (pathmap) ? [
      {
        ...
        properties: {
          ...
          defaultRewriteRuleSet: rewriteRuleSetRef
        }
      }
    ]:null
    ...
    rewriteRuleSets: resolvedRewriteRuleSets
